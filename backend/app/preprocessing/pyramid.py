...
class LunarKeyPoint(cv2.KeyPoint):
    ...
    def __init__(self, x=0.0, y=0.0, size=0.0, angle=-1.0, response=0.0,
                 octave=0, class_id=-1, pyramid_level=0):
        super().__init__(
            x=float(x), y=float(y), size=float(size), angle=float(angle),
...
def _deduplicate_numpy(
    ...
    for idx in order:
        ...
        px, py = points[idx]
        # Brute-force distance check — fast enough for typical keypoint counts (< 2000)
        dx = points[:, 0] - px
        dy = points[:, 1] - py
...
def extract_rift2_multiscale(
    ...
    for level_index, pc_level in enumerate(levels):
        ...
        for kp in keypoints:
            entries.append((kp.pt[0] * scale, kp.pt[1] * scale, max(float(kp.response), 1.0), level_index))
    ...
    return _deduplicate_numpy(entries, np.vstack(descriptor_parts), dedup_radius)
